...
export default function Subscription() {
  ...
  const calculateTotal = () => {
    if (!formData.planId || formData.mealTypes.length === 0 || formData.deliveryDays.length === 0) {
      return 0
    }
    
    const selectedPlan = mealPlans.find(plan => plan.id === parseInt(formData.planId))
  ...
  const validateForm = () => {
    ...
    if (formData.mealTypes.length === 0) newErrors.mealTypes = 'Select at least one meal type'
    if (formData.deliveryDays.length === 0) newErrors.deliveryDays = 'Select at least one delivery day'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Programmer e lagi habis ide')
    } finally {
      ...
      <form onSubmit={handleSubmit} className="subscription-form">
        ...
        <div className="form-section">
              ...
              {mealPlans.map(plan => (
                <div key={plan.id} className="plan-option">
                  ...
                  <label htmlFor={`plan-${plan.id}`}>
                    <strong>{plan.name}</strong> - Rp{plan.price.toLocaleString('id-ID')}/meal
                  </label>
                </div>
              ))}
            </div>
          ...
          <div className="form-group">
            ...
            {errors.mealTypes && <span className="error-message">{errors.mealTypes}</span>}
          </div>
            ...
            <div className="day-selector">
              {daysOfWeek.map(day => (
                <button
                  key={day}
                  type="button"
                  className={`day-button ${formData.deliveryDays.includes(day) ? 'selected' : ''}`}
        ...
        <div className="summary-section">
          ...
          <div className="summary-details">
            <p>
              <strong>Selected Plan:</strong> {formData.planId 
                ? mealPlans.find(p => p.id === parseInt(formData.planId))?.name 
                : 'Not selected'}
            </p>
            <p>
              <strong>Meal Types:</strong> {formData.mealTypes.length > 0 
                ? formData.mealTypes.join(', ') 
                : 'None selected'}
            </p>
            <p>
              <strong>Delivery Days:</strong> {formData.deliveryDays.length > 0 
                ? formData.deliveryDays.join(', ') 
                : 'None selected'}
            </p>
            <p className="total-price">
              <strong>Total Price:</strong> Rp{calculateTotal().toLocaleString('id-ID')}
            </p>
          </div>
        </div>
